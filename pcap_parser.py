...
def flow_rebuild():
        ...
        if eth.type==dpkt.ethernet.ETH_TYPE_IP:
            ...
            if ip.p == dpkt.ip.IP_PROTO_TCP:
                ...
                if not (((ip.src, ip.dst, tcp.sport, tcp.dport) in TCP_flow_pc) or ((ip.dst, ip.src, tcp.dport, tcp.sport) in TCP_flow_pc)):
                    ...
                    TCP_flow_bs[(ip.src, ip.dst, tcp.sport, tcp.dport)] = [Flow(wirelen, ts)]
    ...
    plotFlow(TCP_flow_pc, 500, 10, "TCP", "packet")
    plotFlow(TCP_flow_bs, 400000, 100, "TCP", "byte")

    plotFlow(UDP_flow_pc, 50, 1, "UDP", "packet")
    plotFlow(UDP_flow_bs, 4000, 1, "UDP", "byte")
